                                    ...
                                    ...)
{
  ...
  if (is.null(datafile))
  {
    ...
    #write.table(labeledLoc, paste(
    paste("OccurrenceDatabase",
          spp,
  ...
  if (downloadOnly == FALSE)
  {
    ...
    removed = labeledLoc[(rownames(labeledLoc)
                          %in% rows_purged), ]
    labeledLoc = labeledLoc[!(rownames(labeledLoc)
                              %in% rows_purged), ]
          ...
          for (rasterA_i in 2:length(validRasters))
          {
            for (rasterB_i in 2:length(validRasters))
            {
              if (rasterA_i > rasterB_i)
              {
                ...
                validRasterList = densityRasterRemoveIntersection(densA =
                                                                    densA,
                                                                  densB = densB,
                                                                  verbose = F)
      ...
      } else if (method == "polygon")
      {
        # if(method == "polygon")
        {
          ...
          if (restrictNominate == T)
          {
            ...
            polygons_notnom = densityPolygons[!(names(densityPolygons)
                                                %in% c(nominateSubspecies, "unknown"))]
...
raster::plot(
  bgLayer,col = "grey",colNA = "darkgrey",main = paste("Polygon, subspp:", name)
)
...
dev.off()
            ...
            for (i in 1:length(densityPolygons))
            {
              name = names(densityPolygons)[[i]]
              #sp::spplot(
              densityPolygons[[i]], add = T, border = cols[i], lwd = ((3 * i)
                                                                      / 3)
              )
plot(
  densityPolygons[[i]],add = T,border = cols[i],lwd =((3 * i)
          ...
          ## there is a bug -- if one subspp range is entirely subsumed within another polygon,## will delete that subspecies. no bueno
          ...
          # if(method == "polygon")
          {
            ...
            #   if(length(colnames(polyLocations))
            > 6)
{
...
#
          ...
          {
            #
            #   density_stack = stack(densityRasters[names(densityRasters)
            != "unknown"])
#   density_max = max(density_stack, na.rm = T)
#   density_sum = sum(density_stack, na.rm = T)
#   max_density_list = lapply(
1:length(densityRasters),FUN = function(i)
{
  #     ras = densityRasters[[i]]
  #     #plot(ras, colNA = "grey")
  #     ras2 = density_max-ras
  #     values(ras2)[values(ras2)
  != 0] = NA
...
#   max_density_stack = stack(max_density_list[names(max_density_list)
!= "unknown"])
